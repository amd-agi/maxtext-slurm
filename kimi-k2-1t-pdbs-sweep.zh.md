...
| `sparse-gmm-deepep`    | — *(默认分支)*                                                         | `sparse_matmul=true use_turbo_grouped_gemm=true use_deepep_dispatch=true` — v1      |
| `sparse-gmm-deepep-v2` | `MAXTEXT_PATCH_BRANCH=yihuang/moe-turbo-gmm-and-deepep-v2`              | 与 v1 相同的透传参数                                                                |
| `sparse-gmm-deepep-v3` | `MAXTEXT_PATCH_BRANCH=yihuang/moe-turbo-gmm-and-deepep-v3`              | 与 v1 相同的透传参数                                                                |
...
8 节点中有 2 个（`chi2810`、`chi2872`）的**路由 RoCE GID 在 sysfs 上位于索引 2，而不是索引 1**，其他 6 个节点的路由 GID 都在索引 1：
...
MaxText 在 `base.yml` 中把 `jax_distributed_heartbeat_timeout_seconds` 设为 100 —— 远紧于 JAX 的 300 秒默认值。1T 模型上任何 >100 秒的冷编译都会被它直接杀掉。**在 dense-cf2 pdbs=5 失败（13607）后，所有后续提交都加上了 `_env_jax_distributed_heartbeat_timeout_seconds=14400` 透传。** 由于本仓库未启用 JAX 持久化编译缓存，这个 hedge 每次都需要 —— 在这个扫描里不存在"subsequent pdbs 可以复用 warm cache"的情况。
...
**每次提交都加上 `_env_jax_distributed_heartbeat_timeout_seconds=14400`** —— MaxText 默认 100 秒心跳比绝大多数本模型上的冷编译都紧。除非该单元格已被观测到 <15 min 完成编译，否则用 `--time=45:00` 而不是更短。重试升级阶梯与消歧优先级见 [`moe-pdbs-sweep-prompt.md`](moe-pdbs-sweep-prompt.md)。
