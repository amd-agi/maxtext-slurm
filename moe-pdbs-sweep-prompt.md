...
- **AVAILABLE_NODES**: sorted list of currently-healthy node names in PARTITION, e.g. `chi2766,chi2800,chi2810,chi2832,chi2835,chi2865,chi2872,chi2883`. Run `sinfo -p <PARTITION>` up front to determine this.
...
**Mid-training hang** (silent >3 min after some steps, **no** transient-infra signature in log): first check OOM-hang criterion — if this pdbs > last_successful AND hang is at early step (0 or 1), route to OOM-hang rule. Otherwise: `scancel`, mark `hung@step=N`, no retry (deterministic hang precedent from DS3 step-22).
...
| `heartbeat timeout` / `stop sending heartbeats` / `missed heartbeat` | heartbeat | retry (max 3); add `_env_jax_distributed_heartbeat_timeout_seconds=14400` on retry if cold-compile cause |
...
**Always include `_env_jax_distributed_heartbeat_timeout_seconds=14400` on every submission.** Two reasons:
...
```
_env_jax_distributed_heartbeat_timeout_seconds=14400
```
...
Bad (passthrough):
```bash
./submit.sh <MODEL_TAG>:... -- ... _env_MAXTEXT_PATCH_BRANCH=yihuang/moe-turbo-gmm-and-deepep-v3
# → outputs/NNNNN-JAX-...-_env_MAXTEXT_PATCH_BRANCH_yihuang_moe-turbo-gmm-and-deepep-v3/
```
Good (env-var prefix; only override what differs from `container_env.sh` defaults):
```bash
MAXTEXT_PATCH_BRANCH=yihuang/moe-turbo-gmm-and-deepep-v3 ./submit.sh <MODEL_TAG>:sgd-deepep-v3 ... -- ...
# → outputs/NNNNN-JAX-<MODEL_TAG>-sgd-deepep-v3-.../
```

For v1 runs you don't need any env-var prefix — `container_env.sh` already defaults to the v1 branch. `DOCKER_IMAGE=...` is also defaulted there; only override if testing a different image.
...
| `sparse-gmm-deepep` | — *(default branch)* | `sparse_matmul=true use_turbo_grouped_gemm=true use_deepep_dispatch=true` — v1 |
| **`sparse-gmm-deepep-v2`** | **`MAXTEXT_PATCH_BRANCH=yihuang/moe-turbo-gmm-and-deepep-v2`** | same passthroughs as v1 |
| **`sparse-gmm-deepep-v3`** | **`MAXTEXT_PATCH_BRANCH=yihuang/moe-turbo-gmm-and-deepep-v3`** | same passthroughs as v1 |
...
v1 (dense-like compile budget, baseline branch):
```bash
...
v3 (env-var override for the branch + heartbeat hedge):
```bash
...
Keep tags short and human (`sgd-deepep-v3`) — do NOT bake the branch string or pdbs value into the tag. The env-var prefix carries the branch info; pdbs comes from the passthrough.
...
RAY=1 ./submit.sh <MODEL_TAG>:sgd-deepep-v3 \
  ...
  _env_ENABLE_XLA_DUMP=1 _env_jax_distributed_heartbeat_timeout_seconds=14400
```
...
**Synthetic only.** Pass `dataset_type=synthetic`. **Do NOT attempt grain/c4** — there is a known deterministic DeepEP-specific step-22 hang in that path (see `outputs/13478-*.log`, `outputs/13481-diag/` for the gdb/py-spy evidence from the DS3 sweep). Unresolved. Real-data runs are a separate investigation.
