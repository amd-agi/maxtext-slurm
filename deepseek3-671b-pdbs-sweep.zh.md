...
4. **最佳 sparse 路径：1-node/proc 上的 `sparse-gmm-deepep-v3`**（2026-04-22 后；详见结论 #9）。
   ...
   - **与 `sparse-gmm-fixed` 的部署形态完全一致**：1-node/proc 启动器、相同的 Docker 镜像、相同的透传参数 —— 唯一差异是打补丁的 MaxText 分支（`yihuang/moe-turbo-gmm-and-deepep-v3`）。无需新的 XLA 标志，无需更换启动器。
...
方案 4 在本工作负载下取代了方案 1、2 和 3：pdbs=6 下比方案 3 快 +8.5 %，pdbs=7 下快 +10.9 %（1097 vs 989），保持默认显存预算，并且无需调高 `MEM_FRACTION` 就能扩展到 pdbs=7 的 dropless 峰值。它的部署形态与方案 3 完全相同（1-node/proc 启动器、一个环境变量 —— 即指向 `yihuang/moe-turbo-gmm-and-deepep-v3` 补丁分支 —— 不需要额外的 Docker 镜像）。
...
# 示例：sparse-gmm-deepep, 1-node/proc, pdbs=6
./submit.sh deepseek3-671b --partition=k8s --nodes=8 -- \
