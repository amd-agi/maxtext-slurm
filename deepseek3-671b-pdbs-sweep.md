...
4. **Best sparse path: `sparse-gmm-deepep-v3` on 1-node/proc** (post-2026-04-22; see takeaway #9).
   ...
   - **Same deployment shape as `sparse-gmm-fixed`**: 1-node/proc launcher, same Docker image, same passthrough flags — only difference is the patched MaxText branch (`yihuang/moe-turbo-gmm-and-deepep-v3`). No new XLA flag, no launcher change.
...
Option 4 now supersedes options 1, 2 and 3 on this workload: beats option 3 by +8.5 % TGS at pdbs=6 and +10.9 % at pdbs=7 (1097 vs 989), keeps the default HBM budget, and extends the peak-dropless row without any memory-fraction tuning. It shares the same deployment shape as option 3 (1-node/proc launcher, one env var — in this case the `yihuang/moe-turbo-gmm-and-deepep-v3` patch branch — no extra Docker image).
...
# Example: sparse-gmm-deepep, 1-node/proc, pdbs=6
./submit.sh deepseek3-671b --partition=k8s --nodes=8 -- \
